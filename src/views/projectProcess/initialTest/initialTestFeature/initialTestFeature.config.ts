...
/** 过程截图表单项（可靠性在默认表单后追加；性能效率同时替代测试截图） */
const PROCESS_SCREENSHOT_ITEM: ScBaseFormItem = {
  label: '过程截图',
...
/** 时间特性分析表单项（性能效率专用） */
const TIME_ANALYSIS_ITEM: ScBaseFormItem = {
  label: '时间特性分析',
  prop: 'caseTableType',
...
/** 可靠性表单项工厂（默认表单后追加过程截图） */
const createReliabilityFormItems = () => [
  ...createDefaultFormItems(),
  PROCESS_SCREENSHOT_ITEM
]

/** 性能效率表单项工厂（去测试截图，追加过程截图与时间特性分析表） */
const createPerformanceFormItems = () => [
  ...createUserDocFormItems(),
  PROCESS_SCREENSHOT_ITEM,
  TIME_ANALYSIS_ITEM
]
...
export const FEATURE_CONFIG: Record<FeatureKey, FeaturePageConfig> = {
  ...
  '4': {
    ...
    createFormItems: createReliabilityFormItems,
  ...
  '9': {
    ...
    createFormItems: createPerformanceFormItems,
