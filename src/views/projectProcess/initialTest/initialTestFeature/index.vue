...
import TimeAnalysisEditTable from './components/TimeAnalysisEditTable.vue'
import {
  FEATURE_CONFIG,
  FEATURE_IMPORT_URL,
  FEATURE_SCREENSHOT_UPLOAD_URL,
  INITIAL_TEST_FEATURE_FLAT_BUTTONS,
  createInitialTestFeatureFormData
} from './initialTestFeature.config'
import type { FeatureKey } from './initialTestFeature.config'
...
const searchbarItems = reactive<SearchbarItems<InitialTestFeatureSearchParams>>(
  [
...
const dialogFormData = createInitialTestFeatureFormData()

const { visible, formData, confirmLoading, open, handleConfirm, dialogTitle } =
  useDialogForm<InitialTestFeatureFormData, 'id', string, FormData>({
    ...
    fetchDetail: async id => {
      ...
      // 性能效率：详情 timeAnalysisList 回填至表单 assetAcceptanceTimeAnalysisList（源 assignByMap 映射）
      ...
      return {
        ...res,
        data: result
      } as DataResponse<InitialTestFeatureFormData>
    },
...
// subFeature 选项随字典过滤结果注入；是否启用方案合并时移除「测试项说明」
...
const pageConfig = computed<PageConfig<InitialTestFeatureResult>>(() => ({
  searchConfig: { searchbarItems },
  operateConfig: {
    // 显式置空防缺省回落 ['add']；全部操作按钮经 #operate-button-slot 渲染（systemDetails 同款）
    defaultButtons: []
  },
  tableConfig: {
    get tableColumns() {
...
const { handleDelete } = useDeleteAction<InitialTestFeatureResult>(
...
/**
 * 常规导入统一走自定义上传，读取请求时刻的 feature 配置（切换特性无陈旧参数）：
 * 可靠性 file/80 分流与性能 250 字段在此构建 FormData；
 * 标准特性退化为公共默认上传（单文件固定 file 字段）
 */
...
/**
 * 「数据导入导出操作」下拉菜单项（systemDetails 同款四项布局）：
    ...
    >
      <template #operate-button-slot>
        <OperateButtonGroup
          ...
          :flat-buttons="INITIAL_TEST_FEATURE_FLAT_BUTTONS"
