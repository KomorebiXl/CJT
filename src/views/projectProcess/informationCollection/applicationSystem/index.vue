...
const searchbarItems = reactive<SearchbarItems<ApplicationSystemSearchParams>>([
  {
    label: '资产编号',
    prop: 'assetNo',
    type: 'input',
...
const pageConfig: PageConfig<ApplicationSystemData> = {
  ...
  fetchData: getApplicationSystemDataAPI
}
...
const defaultFormData = reactive<ApplicationSystemFormData>({
  assetNo: '',
  assetName: '',
  assetSecondType: '',
  assetAddress: '',
...
const formItems = defineFormItems<ApplicationSystemFormData>([
  {
    label: '系统资产编号',
    prop: 'assetNo',
    type: 'input'
...
const handlePageClick = (row: ApplicationSystemData | undefined = undefined) =>
  open(row)

const { handleDelete } = useDeleteAction<ApplicationSystemData>(
  ids => deleteApplicationSystemAPI({ ids }),
  {
    message: '确定删除该应用系统吗？删除后不可恢复。',
...
onMounted(() => {
  Promise.allSettled([loadAssetSecondTypeOptions(), loadGuideOptions()])
})

const { visible, formData, confirmLoading, open, handleConfirm, dialogTitle } =
  useDialogForm<ApplicationSystemFormData>({
    defaultFormData,
    title: '应用系统',
    fetchDetail: id => getApplicationSystemDetailAPI(id),
    onCreate: data => createApplicationSystemAPI(data),
    onUpdate: data => updateApplicationSystemAPI(data),
    onSuccess: () => scResourcePageRef.value?.refresh()
  })
