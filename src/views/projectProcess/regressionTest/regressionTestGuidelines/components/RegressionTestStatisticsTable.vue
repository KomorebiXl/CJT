...
import { getRegressionTestStatisticsAPI } from '@/api/projectProcess/regressionTest-api.ts'
...
/** 回归统计不消费 envNoDo，无「环境不具备」行（区别于首轮/方案统计表） */
type FeatureStat = {
  total: number
  unFinish: number
  conformity: number
...
const transformToTableData = (list: RegressionTestStatisticsData[]) => {
  columns.value = [
...
const fetchStatistics = async () => {
  const [err, res] = await safeRequest(getRegressionTestStatisticsAPI())
  if (err || !res) return
