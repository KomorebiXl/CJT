  ...
  >
    <el-scrollbar max-height="500px">
      ...
      >
        ...
        <template #custom-concurrent="{ data: row }">{{
          row.concurrent === '1' || row.concurrent === 1 ? '禁止' : '允许'
        }}</template>
        <template #custom-misfirePolicy="{ data: row }">{{
          ['默认策略', '立即执行', '执行一次', '放弃执行'][
            Number(row.misfirePolicy) ?? 0
          ] ?? value(row.misfirePolicy)
        }}</template>
      </ScBaseForm>
    </el-scrollbar>
    <template #footer
      ><ScButton type="warning" @click="modelValue = false"
        >关闭</ScButton
      ></template
    >
  </ScDialog>
